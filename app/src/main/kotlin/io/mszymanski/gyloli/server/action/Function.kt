...
/**
 * A named piece of JavaScript a team wrote, callable from an action.
 *
...
interface WorkflowFunctionRepository : JpaRepository<WorkflowFunction, Long> {
    ...
    fun findByTeamId(teamId: Long, pageable: Pageable): Page<WorkflowFunction>

    fun findByTeamId(teamId: Long): List<WorkflowFunction>

    fun findByTeamIdAndName(teamId: Long, name: String): WorkflowFunction?
}
...
class FunctionNameTakenException(name: String) :
    RuntimeException("A function named \"$name\" already exists in this team")
