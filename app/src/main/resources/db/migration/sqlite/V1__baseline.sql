...
(
    ...
    constraint ck_connection_type CHECK (((type) IN ('SLACK', 'GITHUB', 'JIRA', 'SMTP', 'WEBHOOK')))
);
...
(
    ...
    retry_backoff                varchar(16),
    attempts                     integer not null default 0,
    constraint uk_execution_step UNIQUE (execution_id, node_key),
    constraint ck_execution_step_retry_backoff CHECK (((retry_backoff IS NULL) OR ((retry_backoff) IN ('FIXED', 'EXPONENTIAL')))),
...
(
    ...
    constraint ck_workflow_node_retry_backoff CHECK (((retry_backoff IS NULL) OR ((retry_backoff) IN ('FIXED', 'EXPONENTIAL')))),
...
(
    ...
    constraint ck_workspace_connection_type CHECK (((type) IN ('SLACK', 'GITHUB', 'JIRA', 'SMTP', 'WEBHOOK'))),
