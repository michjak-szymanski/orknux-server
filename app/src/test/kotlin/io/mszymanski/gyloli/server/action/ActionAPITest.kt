...
class ActionAPITest(
    ...
    fun `an HTTP action reads its inputs from the URL and the headers`() {
        graphQlTester.document(
            ...
            mutation {
              createAction(input: {
                teamId: $teamId, name: "Fetch API Data", type: EXECUTE, subtype: HTTP_REQUEST,
    ...
    fun `a function action answers with what the function returns`() {
        ...
        graphQlTester.document(
            ...
            mutation {
              createAction(input: {
                teamId: $teamId, name: "Transform Data", type: EXECUTE, subtype: FUNCTION,
    ...
    fun `a delay takes a duration and answers with nothing`() {
        graphQlTester.document(
            ...
            mutation {
              createAction(input: {
                teamId: $teamId, name: "Delay Timer", type: WAIT, subtype: TIME, durationSeconds: 60
    ...
    fun `a subtype that belongs to the other type is refused`() {
        graphQlTester.document(
            ...
            mutation {
              createAction(input: {
                teamId: $teamId, name: "Confused", type: WAIT, subtype: HTTP_REQUEST, url: "https://example.com"
              }) { id }
            }
            """,
    ...
    fun `an action without the setting it runs on is refused`() {
        graphQlTester.document(
            ...
            mutation {
              createAction(input: {
                teamId: $teamId, name: "Nowhere", type: EXECUTE, subtype: HTTP_REQUEST
              }) { id }
            }
            """,
    ...
    @Test
    fun `a connection from another team is refused`() {
        val otherTeam = requireNotNull(teams.save(Team(name = "frontend")).id)
        val theirs = graphQlTester.document(
            """
            mutation {
              createTeamConnection(input: {
                teamId: $otherTeam, name: "Theirs", type: SLACK, url: "https://slack.com/api"
              }) { id }
            }
            """,
        ).execute().path("createTeamConnection.id").entity(Long::class.java).get()
        ...
        graphQlTester.document(
            ...
            mutation {
              createAction(input: {
                teamId: $teamId, name: "Borrowed", type: EXECUTE, subtype: OUTGOING_CONNECTION,
                connectionId: $theirs, connectionAction: SEND_MESSAGE
              }) { id }
            }
            """,
        ).execute().errors().expect { it.message?.contains("a connection this team holds") == true }.verify()
    }
    ...
    @Test
    fun `a duplicate name inside a team is refused`() {
        repeat(2) { attempt ->
    ...
    private fun createFunction(name: String, returnType: String): Long = graphQlTester.document(
        """
        mutation {
          createFunction(input: { teamId: $teamId, name: "$name", returnType: $returnType }) { id }
        }
        """,
