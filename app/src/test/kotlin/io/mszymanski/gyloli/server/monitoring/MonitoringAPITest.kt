package io.mszymanski.gyloli.server.monitoring
...
class MonitoringAPITest(
    ...
    fun `reports the service, its version and what it depends on`() {
        graphQlTester.document(
            ...
            .path("components[0].name").entity(String::class.java).isEqualTo("gyloli-server")
            .path("components[0].status").entity(String::class.java).isEqualTo("HEALTHY")
            .path("components[0].version").entity(String::class.java).isEqualTo("1.0.0-SNAPSHOT")
...
/** The screen is behind the organization section, so the check belongs here too. */
@SpringBootTest
@AutoConfigureGraphQlTester
