...
class SessionAPITest(
    ...
    fun `signs in a directory user and reports their roles`() {
        ...
        assertThat(response.body?.email).isEqualTo("alice@gyloli.io")
    ...
    fun `describes the signed-in user and forgets them on sign-out`() {
        ...
        assertThat(current.body?.email).isEqualTo("bob@gyloli.io")
    ...
    fun `rejects graphql calls without a session`() {
        val response = graphql("{ teams { totalElements } }", cookie = null)
    ...
    fun `attributes an audit entry to the signed-in ldap user`() {
        ...
        val response = graphql("""mutation { createTeam(input: { name: "platform" }) { id name } }""", cookie)

        assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
        assertThat(response.body).contains("platform")

        val entry = auditRepository.findAll().single { it.operationType == TeamOperationType.ADD }
        assertThat(entry.userId).isEqualTo("alice")
        assertThat(entry.operationType).isEqualTo(TeamOperationType.ADD)
        assertThat(entry.newTeamName).isEqualTo("platform")
    }
