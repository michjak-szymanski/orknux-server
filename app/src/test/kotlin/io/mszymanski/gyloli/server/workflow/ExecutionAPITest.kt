...
class ExecutionAPITest(
    ...
    fun reset() {
        ...
        workflowId = graphQlTester.document(
            """
            mutation {
              createWorkflow(input: { teamId: $teamId, name: "Data Processing Pipeline" }) { workflowId }
            }
            """,
    ...
    @Test
    fun `a team's runs are listed, and the filters reach the module`() {
        start()

        graphQlTester.document(
            """
            query {
              teamExecutions(teamId: $teamId, search: "pipeline", days: 1) {
            ...
            .path("teamExecutions.totalElements").entity(Int::class.java).isEqualTo(1)

        graphQlTester.document(
            """query { teamExecutions(teamId: $teamId, search: "nothing-like-this") { totalElements } }""",
        ).execute().path("teamExecutions.totalElements").entity(Int::class.java).isEqualTo(0)
    }
    ...
    private fun start() = runs.startExecution(
        StartExecutionInput(teamId = teamId, workflowId = workflowId, trigger = ExecutionTrigger.WEBHOOK),
    )
}
