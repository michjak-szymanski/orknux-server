...
class WorkflowGraphAPITest(
    ...
    @Test
    fun `a trigger node instances a definition from the workspace's catalogue`() {
        val triggerId = trigger(workspaceId, "Slack Mention Handler")
    ...
    @Test
    fun `refuses a definition from another workspace's catalogue`() {
        val theirs = trigger(otherWorkspaceId, "Theirs")
        ...
        graphQlTester.document(
            ...
            """,
        ).execute().errors().expect { it.message?.contains("not in this workspace's catalogue") == true }.verify()
    }

    @Test
    fun `a new workflow starts as an empty draft`() {
        graphQlTester.document(
            """query { workflowGraph(workspaceId: $workspaceId, workflowId: $workflowId) { name status nodes { key } edges { source } } }""",
        ).execute()
    ...
    private fun trigger(workspace: Long, name: String): Long = graphQlTester.document(
        """
        mutation {
          createTrigger(input: { workspaceId: $workspace, name: "$name", type: SCHEDULED, cron: "0 2 * * *" }) { id }
        }
        """,
    ...
    fun `saves nodes and edges, and reads them back`() {
        ...
        val response = graphQlTester.document(
            ...
            query {
              workflowGraph(workspaceId: $workspaceId, workflowId: $workflowId) {
                nodes { key kind name description agentId x y }
                edges { source target }
              }
            }
            """,
        ...
        // An agent node points at one of the workspace's agents; the agent
        // supplies the model and the instructions.
        assertThat(research.agentId).isEqualTo(agentId)
    ...
    fun `publishing needs at least one node, then flips the status`() {
        graphQlTester.document("""mutation { publishWorkflow(workspaceId: $workspaceId, workflowId: $workflowId) { status } }""")
            .execute()
            .errors()
            .satisfy { errors ->
    ...
    fun `editing a published workflow puts it back into draft`() {
        ...
        graphQlTester.document("""query { workflowGraph(workspaceId: $workspaceId, workflowId: $workflowId) { status } }""")
            .execute().path("workflowGraph.status").entity(String::class.java).isEqualTo("DRAFT")
    }

    @Test
    fun `a workspace without the workflow assigned cannot read its graph`() {
        graphQlTester.document(
            """query { workflowGraph(workspaceId: $otherWorkspaceId, workflowId: $workflowId) { status } }""",
        ).execute()
            .errors()
            .satisfy { errors ->
                assertThat(errors).singleElement()
