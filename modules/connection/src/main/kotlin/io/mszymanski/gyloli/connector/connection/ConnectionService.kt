...
/**
 * The organization's default connections. They are provisioned into every team
 * created after they are defined; editing one reaches the teams that hold a copy.
 *
 * Who may call this is settled before the request arrives: gyloli-server checks
...
class ConnectionService(
    ...
    fun createConnection(input: ConnectionInput): CreatedConnectionView {
        ...
        // entry: "Default connection Slack added to 3 existing teams".
        val backfilled = if (input.addToExistingTeams == true) {
            provisioning.provisionToExistingTeams(connection)
        } else {
            0
        }
    ...
    fun updateConnection(id: Long, input: ConnectionInput): ConnectionView {
        ...
        teamConnections.findByConnectionId(id).forEach { held ->
            held.name = name
     ...
     * credentials included, and become the teams' own: tidying the organization
     * list must not take a working integration away from a team.
     */
    @Transactional
    fun deleteConnection(id: Long): Boolean {
        val connection = connections.findByIdOrNull(id) ?: return false

        teamConnections.findByConnectionId(id).forEach { it.connectionId = null }
...
data class ConnectionInput(
    ...
    val url: String,
    /**
     * Defaults reach new teams as they are created; this also gives it to the
     * teams that already exist. Ignored when updating.
     */
    val addToExistingTeams: Boolean? = null,
)
...
/** The new default, plus how many existing teams took it, for the caller's audit entry. */
data class CreatedConnectionView(
    val connection: ConnectionView,
    val addedToTeams: Int,
)
