...
enum class ConnectionType {
     ...
     * app-level token to open the websocket. This is the type gyloli listens
...
/**
 * An organization-wide default connection. New teams are provisioned with a copy
 * of every default; the credentials are always the team's own.
 */
@Entity
 ...
 *
 * [teamId] names a team in gyloli-server; there is no foreign key to enforce it
 * and no cascade to clean it up, so a deleted team is forgotten only when the
 * server tells the connector about it.
 */
@Entity
@Table(name = "team_connection")
class TeamConnection(
    ...
    /** The organization default this was provisioned from; null when team-owned. */
    ...
    /** Used instead of [url] when the team points at its own endpoint. */
    ...
    @CollectionTable(
        name = "team_connection_header",
        joinColumns = [JoinColumn(name = "team_connection_id")],
    )
    ...
    /**
     * What the team screen reports. A connection is only [ConnectionStatus.CONNECTED]
