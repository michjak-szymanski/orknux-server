 ...
 *
 * There is no per-team access check here, unlike gyloli-server: the caller is a
...
class ExecutionService(
    ...
    fun executions(
        ...
        val filter = executionFilter(teamId, workflowId, status, since, search?.trim()?.ifEmpty { null })
        return ExecutionPage(executions.findAll(filter, pageable))
    }

    /** Where a workflow last got to, or null if it has never run for this team. */
    fun lastExecution(teamId: Long, workflowId: Long): ExecutionView? =
        executions.findFirstByTeamIdAndWorkflowIdOrderByStartedAtDesc(teamId, workflowId)?.let(::ExecutionView)
